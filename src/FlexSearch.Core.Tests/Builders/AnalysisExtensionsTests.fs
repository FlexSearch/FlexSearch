...
module Helpers = 
    let GetFactory() = 
        ...
        factory.FilterFactory.GetModuleByName("notexists").Returns(Choice2Of2(Errors.FILTER_NOT_FOUND |> GenerateOperationMessage)) 
        |> ignore
        factory.FilterFactory.GetModuleByName("exists")
               .Returns(Choice1Of2(new StandardFilterFactory() :> IFlexFilterFactory)) |> ignore
        factory.TokenizerFactory.GetModuleByName("notexists").Returns(Choice2Of2(Errors.TOKENIZER_NOT_FOUND |> GenerateOperationMessage)) 
        |> ignore
...
//        sut.Validate(factoryCollection) |> ExpectErrorCode (MessageConstants.FILTER_NOT_FOUND |> GenerateOperationMessage)
...
//        let factoryCollection = Helpers.GetFactory()
//        sut.Validate(factoryCollection) |> ExpectErrorCode MessageConstants.TOKENIZER_NOT_FOUND |> GenerateOperationMessage
