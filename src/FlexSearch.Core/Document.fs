...
module Document = 
    ...
    let Update (document : FlexSearch.Api.Document) flexIndex = 
        ...
        let targetIndex = mapToShard document.Id flexIndex.Shards.Length
        let targetDocument = Generate document flexIndex.IndexSetting
        flexIndex.Shards.[targetIndex].TrackingIndexWriter.updateDocument(new Term(Constants.IdField, document.Id), targetDocument)
...
module IO = 
    ...
    let GetIndexWriterConfig(flexIndexSetting : FlexIndexSetting) = 
        ...
        iwc.setRAMBufferSizeMB (System.Double.Parse(flexIndexSetting.IndexConfiguration.RamBufferSizeMb.ToString())) |> ignore
        Some(iwc)
    ...
    let GetIndexDirectory(directoryPath : string, directoryType : DirectoryType) = 
        ...
        try 
            ...
            | DirectoryType.FileSystem -> 
                Some(FSDirectory.``open`` (file, lockFactory) :> org.apache.lucene.store.Directory)
            | DirectoryType.MemoryMapped -> 
                Some(MMapDirectory.``open`` (file, lockFactory) :> org.apache.lucene.store.Directory)
            | DirectoryType.Ram -> Some(new RAMDirectory() :> org.apache.lucene.store.Directory)
            | _ -> failwithf "Unknown directory type."
        with e -> //logger.Error(sprintf "Unable to open index at location: %s" directoryPath, e); 
...
module FlexIndex = 
    ...
    type IndexService(settingsParser : ISettingsBuilder, keyValueStore : IPersistanceStore, loadAllIndex : bool, versionCache : IVersioningCacheStore) = 
        ...
        let indexExists (indexName) = 
            ...
            | (true, flexIndex) -> 
                ...
                | _ -> 
                    ...
                    let lastModifiedField = new LongField(Constants.LastModifiedField, GetCurrentTimeAsLong(), Field.Store.YES)
                    ...
                    for field in flexIndex.IndexSetting.Fields do
                        ...
                        if (field.FieldName = Constants.IdField || field.FieldName = Constants.TypeField || field.FieldName = Constants.LastModifiedField || field.FieldName = Constants.VersionField) then 
                            ()
                        else 
        ...
        let UpdateDocument(flexIndex : FlexIndex, documentTemplate : threadLocalDocument, documentId : string, 
            ...
            for field in flexIndex.IndexSetting.Fields do
                // Ignore these 3 fields here.
                if (field.FieldName = Constants.IdField || field.FieldName = Constants.TypeField || field.FieldName = Constants.LastModifiedField 
                    || field.FieldName = Constants.VersionField) then ()
                else 
