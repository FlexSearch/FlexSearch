...
module Socket = 
    ...
    type TcpSocketServer(port : int, requestHandler) = 
        ...
        let config = 
            new ServerConfig(Port = port, Ip = "127.0.0.1", MaxConnectionNumber = 1000, Mode = SocketMode.Tcp, 
                             Name = "FlexTcpServer") :> IServerConfig
        let mutable server = Unchecked.defaultof<ProtoBufferServer>
        
        do 
            try 
                server <- new ProtoBufferServer()
                if server.Setup(config, null, null, new ConsoleLogFactory()) <> true then 
                    failwith "Unable to setup the server."
                server.add_NewRequestReceived (requestHandler)
            with ex -> Console.Write(ex.Message)
    ...
    type TcpClient(ipAddress : System.Net.IPAddress, port : int) = 
        ...
        member this.Send(messageType : byte, data : byte []) = 
            ...
            message.Add(new ArraySegment<byte>([| (byte) (data.Length / 256)
