...
type FlexAnalyzerBuilderTests() = 
    ...
    member __.``Should build successfully for a known filter`` (sut : Analyzer, filter : TokenFilter) = 
        sut.Filters.Clear()
    ...
    member __.``Should not build successfully for an unknown filter`` (sut : Analyzer, filter : TokenFilter) = 
        sut.Filters.Clear()
    ...
    member __.``Should build a analyzer using synonym filter`` (sut : Analyzer, filter : TokenFilter) =
        sut.Filters.Clear()
